/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { forwardRef, ChangeEvent } from 'react';
import {
  DISCOUNT_LABEL_SIZE,
  LOGO_X_OFFSET,
  LOGO_Y_OFFSET,
  UP_TO_OFF_COLOR,
  TITLE_Y_OFFSET,
  TITLE_MAX_WIDTH,
  DISCOUNT_PERCENT_FONT_SIZE,
  THEMES,
  BANNER_RADIUS,
  SALE_NAME_X_OFFSET,
  SALE_NAME_Y_OFFSET,
  SALE_NAME_MAX_HEIGHT
} from '../constants';
import { PricingCard } from './PricingCard';
import { Logo } from './Logo';
import { PricingPlan, TabType } from '../types';

export const BannerCanvas = forwardRef<HTMLDivElement, {
  discountPercent: string;
  bgImage: string | null;
  plans: PricingPlan[];
  sectionTitle: string;
  mode?: TabType;
  saleNameImage?: string | null;
  onUploadSaleNameImage?: (event: ChangeEvent<HTMLInputElement>) => void;
}>(({ discountPercent, bgImage, plans, sectionTitle, mode = 'forex', saleNameImage = null, onUploadSaleNameImage }, ref) => {
  const theme = THEMES[mode || 'forex'];
  const accentColor = theme.accent;

  const labelStyle = {
    fontSize: `${DISCOUNT_LABEL_SIZE}vw`,
    lineHeight: '1.2',
    color: UP_TO_OFF_COLOR
  };

  // Logo is 20% smaller → was h-[2vw], now h-[1.6vw]
  const logoStyle = {
    transform: `translate(${LOGO_X_OFFSET}vw, ${LOGO_Y_OFFSET}vw)`
  };

  const titleStyle = {
    transform: `translateY(${TITLE_Y_OFFSET}vw)`
  };

  // Filter plans based on mode
  let filteredPlans = plans;
  if (mode === 'algo') {
    filteredPlans = plans.filter(p => p.id === 'rise' || p.id === 'advance');
  } else if (['copier', 'guardian', 'raven'].includes(mode || '')) {
    filteredPlans = plans.filter(p => p.id === 'advance');
  }

  return (
    <div className="flex-1 flex items-center justify-center p-8 bg-[#000]">
      <div
        ref={ref}
        className="aspect-video w-full max-w-[1280px] relative overflow-hidden flex ring-1 ring-white/10"
        style={{
          background: `linear-gradient(135deg, ${theme.bg.join(', ')})`,
          borderRadius: `${BANNER_RADIUS}vw`
        }}
      >
        {/* Background Layer */}
        {bgImage ? (
          <img
            src={bgImage}
            alt="Background"
            className="absolute inset-0 w-full h-full object-cover opacity-50 z-0"
            referrerPolicy="no-referrer"
          />
        ) : (
          <div className="absolute inset-0 bg-grid opacity-40 pointer-events-none" />
        )}

        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-transparent to-transparent z-0" />

        {/* Column 1: Sales Info */}
        <div className="w-[38%] flex flex-col p-[4%] z-10 justify-between">
          {/* Logo — 20% smaller: h-[1.6vw] */}
          <div style={logoStyle}>
            <Logo className="h-[1.6vw] w-auto" />
          </div>

          {/* Sale Name Image Area */}
          <div className="flex flex-col flex-1 justify-center" style={titleStyle}>
            {saleNameImage ? (
              <img
                src={saleNameImage}
                alt="Sale Name"
                className="max-w-full object-contain"
                style={{
                  maxHeight: `${SALE_NAME_MAX_HEIGHT}vw`,
                  transform: `translate(${SALE_NAME_X_OFFSET}vw, ${SALE_NAME_Y_OFFSET}vw)`
                }}
              />
            ) : (
              <label
                className="group flex flex-col items-center justify-center cursor-pointer border-[0.15vw] border-dashed border-white/20 hover:border-white/40 hover:bg-white/5 transition-all"
                style={{
                  borderRadius: '0.5vw',
                  padding: '2.5vw 3vw',
                  maxWidth: `${TITLE_MAX_WIDTH}%`,
                  minHeight: '13vw',
                  gap: '0.8vw'
                }}
              >
                <input
                  type="file"
                  accept="image/png,image/*"
                  onChange={onUploadSaleNameImage}
                  className="hidden"
                />
                <svg
                  className="opacity-30 group-hover:opacity-60 transition-opacity"
                  style={{ width: '3.5vw', height: '3.5vw' }}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="white"
                  strokeWidth="1.5"
                >
                  <path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <span
                  className="font-black uppercase tracking-widest text-center text-white/25 group-hover:text-white/50 transition-colors"
                  style={{ fontSize: '0.9vw' }}
                >
                  Upload Sale Name PNG
                </span>
              </label>
            )}
          </div>

          {/* Discount % */}
          <div
            className="flex flex-col font-black italic uppercase leading-none select-none"
            style={{
              filter: 'drop-shadow(0.15vw 0.15vw 0.2vw rgba(0,0,0,0.6))'
            }}
          >
            <span
              style={{
                color: '#ffffff',
                fontSize: '1.8vw',
                letterSpacing: '0.04em',
                lineHeight: '1',
                marginBottom: '0.3vw'
              }}
            >
              SAVE UPTO
            </span>
            <span
              className="whitespace-nowrap bg-clip-text text-transparent bg-gradient-to-r from-[#0088ff] via-[#00e5ff] to-[#00ffb0] leading-none"
              style={{
                fontSize: '4.2vw',
                letterSpacing: '-0.05em',
              }}
            >
              {discountPercent}% OFF
            </span>
          </div>
        </div>

        {/* Column 2: Subscription Plans */}
        <div className="w-[62%] flex flex-col p-[4%] z-10">
          <div className="flex w-full justify-center mb-[2%]" style={titleStyle}>
            <div className="flex flex-col items-center">
              <h2
                className="font-orbitron font-bold uppercase tracking-[0.1em] leading-tight"
                style={{
                  color: accentColor,
                  fontSize: '3.5vw',
                  textShadow: '0 0 20px rgba(0,0,0,0.5)'
                }}
              >
                {sectionTitle.includes('Subscription')
                  ? sectionTitle.split('Subscription')[0].trim()
                  : sectionTitle.split(' ')[0]}
              </h2>
              <h3
                className="font-orbitron font-medium uppercase tracking-[.1em] leading-none mt-[-0.1vw]"
                style={{
                  color: '#ffffff',
                  fontSize: '1.5vw',
                  opacity: 0.9
                }}
              >
                {sectionTitle.includes('Subscription')
                  ? `Subscription ${sectionTitle.split('Subscription')[1].trim()}`.trim()
                  : sectionTitle.split(' ').slice(1).join(' ')}
              </h3>
            </div>
          </div>

          <div className="flex-1 flex items-center justify-center space-x-[3%] pb-[2%]">
            {filteredPlans.map(plan => (
              <PricingCard key={plan.id} plan={plan} mode={mode} />
            ))}
          </div>
        </div>
      </div>
    </div>
  );
});
